...
def send_updates(
    ...
    headers = {}
    # API_TOKEN is a shared HMAC secret generated by the server at startup and injected into
    # Ray worker processes via environment variable. It authenticates as the __internal__
    # pseudo-user (global admin). Workers running on separate machines should communicate
    # over TLS (e.g. behind a reverse proxy) to protect this token in transit.
