...
class DataRequestBody(BaseModel):
    task_index: int = Field(
        ...,
        description="The index of the task in the project tasks list for which data is requested.",
    )
    view: ViewParamTypes = Field(
        default=ViewParams(name="identity"),
        description="The view parameters to apply to the data before returning it.",
    )
...
async def get_data(
    ...
    data_loader: DataLoader = LoaderRegistry.get(project.data_loader.name)()

    try:
        data = data_loader.get_sample(
            sample,
            project.tasks[task_index].signal_names,
