...
def test_model_train_predict(server_setup, setup_model_samples, page: Page, model_name):
    ...
    if model_name == "mock_params_timeseries_cnn":
        ...
        page.get_by_role("textbox", name="Final Score").fill(
            "Test Text in Number Field"
        )
        ...
        expect(
            page.get_by_role("option", name="selection_2", exact=True)
        ).to_be_visible()
        ...
        expect(
            page.get_by_text("There are problems with your submission:")
        ).to_be_visible()
