...
async def test_annotators_invalid_type(api_client, setup_db):
    ...
    response = await api_client.post(f"/projects/{setup_db['project_id_1']}/samples/{setup_db['sample_id_3']}/annotator/wrong_type", json=params)
...
async def test_annotators_not_supported_for_project(api_client, setup_db):
    ...
    response = await api_client.post(f"/projects/{setup_db['project_id_1']}/samples/{setup_db['sample_id_3']}/annotator/find_peaks", json=params)
...
async def test_annotators(api_client, setup_db):
    ...
    response = await api_client.post(f"/projects/{setup_db['project_id_2']}/samples/{setup_db['sample_id_4']}/annotator/find_peaks", json=params)
    ...
    assert all(annotation.get("label") == "ELM" for annotation in returned_annotations)
