...
async def test_insert(db_client):
    ...
    retrieved_project = await db_client.db["projects"].find_one({"_id": ObjectId(project_id)})
    assert retrieved_project["name"] == PROJECT_1.name
    
@pytest.mark.asyncio
async def test_insert_ids(db_client):
    project_id = await db_client.insert(collection="projects", model=PROJECT_1)
    sample_id = await db_client.insert(collection="samples", model=SAMPLE_1, ids={"project_id": ObjectId(project_id)})
    
    # Check sample has been inserted with project ID attached
    retrieved_sample = await db_client.db["samples"].find_one({"_id": ObjectId(sample_id)})
...
async def test_insert_many(db_client):
    project_ids = await db_client.insert_many(collection="projects", models=[PROJECT_1, PROJECT_2])
    
    # Check projects has been inserted
    retrieved_project_1 = await db_client.db["projects"].find_one({"_id": ObjectId(project_ids[0])})
    assert retrieved_project_1["name"] == PROJECT_1.name
    
    retrieved_project_2 = await db_client.db["projects"].find_one({"_id": ObjectId(project_ids[1])})
...
async def test_insert_many_same_ids(db_client):
    project_id = await db_client.insert(collection="projects", model=PROJECT_1)
    sample_ids = await db_client.insert_many(collection="samples", models=[SAMPLE_1, SAMPLE_2], ids={"project_id": ObjectId(project_id)})
    
    # Check samples has been inserted with project ID attached
    retrieved_sample_1 = await db_client.db["samples"].find_one({"_id": ObjectId(sample_ids[0])})
    
    assert retrieved_sample_1["shot_id"] == SAMPLE_1.shot_id
    assert retrieved_sample_1["project_id"] == ObjectId(project_id)
    
    retrieved_sample_2 = await db_client.db["samples"].find_one({"_id": ObjectId(sample_ids[1])})
...
async def test_insert_many_different_ids(db_client):
    project_ids = await db_client.insert_many(collection="projects", models=[PROJECT_1, PROJECT_2])
    sample_ids = await db_client.insert_many(collection="samples", models=[SAMPLE_1, SAMPLE_2], ids=[{"project_id": ObjectId(project_ids[0])}, {"project_id": ObjectId(project_ids[1])}])
...
async def test_get_document_by_id(db_client):
    ...
    returned_item = await db_client.get_document_by_id(collection="projects", object_id = result.inserted_id)
...
async def test_get_document_by_id_doesnt_exist(db_client):
    returned_item = await db_client.get_document_by_id(collection="projects", object_id = ObjectId())
...
async def test_get_all_documents(db_client):
    ...
    await db_client.db["samples"].insert_one({"name":"Test Sample"}) # <--- shouldn't include this
...
async def test_get_filtered_documents_ascending(db_client, setup_db_for_filtering):
    ...
    results = await db_client.get_filtered_documents("projects", sort_direction="ascending")
...
async def test_get_filtered_documents_filter(db_client, setup_db_for_filtering):
    results = await db_client.get_filtered_documents("projects", filters={"local": True})
...
async def test_delete_filtered_documents_filters(db_client, setup_db_for_filtering):
    results = await db_client.delete_filtered_documents("projects", filters={"local": True})
