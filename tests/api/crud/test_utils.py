...
async def test_get_projects(db_client, setup_db):
    ...
    assert [project["name"] for project in projects] == ["project_2", "test_project_1", "test_project_0"]
...
async def test_get_projects_by_name(db_client, setup_db):
    ...
    assert [project["name"] for project in projects] == ["test_project_1", "test_project_0"]
...
async def test_get_project_wrong_id(db_client, setup_db):
    with pytest.raises(HTTPException, match="Project not found with that ID"):
        project = await utils.get_project(db_client, str(ObjectId()))
...
async def test_get_samples_by_shot_id(db_client, setup_db):
    samples = await utils.get_samples(db_client, project_id=setup_db["project_id_1"], shot_id=3)
...
async def test_get_sample(db_client, setup_db):
    sample = await utils.get_sample(db_client, setup_db["project_id_1"], setup_db["sample_id_1"])
...
async def test_get_sample_wrong_id(db_client, setup_db):
    with pytest.raises(HTTPException, match="Sample not found with that ID"):
        sample = await utils.get_sample(db_client, setup_db["project_id_1"], str(ObjectId()))
...
async def test_delete_specific_sample(db_client, setup_db):
    # Delete all samples belonging to a project
    await utils.delete_samples(db_client, project_id=setup_db["project_id_1"], sample_id=setup_db["sample_id_1"])
    # Should be three remaining sample
    samples = await db_client.get_filtered_documents("samples")
    assert len(samples) == 3
...
async def test_get_annotations_in_project(db_client, setup_db):
    annotations = await utils.get_annotations(db_client, project_id=setup_db["project_id_1"])
...
async def test_get_annotations_in_sample(db_client, setup_db):
    annotations = await utils.get_annotations(db_client, project_id=setup_db["project_id_1"], sample_id=setup_db["sample_id_1"])
...
async def test_add_annotations(db_client):
    ...
    annotation_ids = await utils.add_annotations(db_client, project_id=project_id, sample_id=sample_id, annotations=[ANNOTATION_1, ANNOTATION_2])
...
async def test_delete_all_annotations_in_project_and_sample(db_client, setup_db):
    ...
    await utils.delete_annotations(db_client, project_id=setup_db["project_id_1"], sample_id=setup_db["sample_id_1"])
...
async def test_delete_specific_annotation(db_client, setup_db):
    ...
    await utils.delete_annotations(db_client, project_id=setup_db["project_id_1"], sample_id=setup_db["sample_id_1"], annotation_id=setup_db["annotation_id_1"])
    # Should be four remaining annotations
    annotations = await db_client.get_filtered_documents("annotations")
    assert len(annotations) == 4
    assert setup_db["annotation_id_1"] not in [str(annotation["_id"] for annotation in annotations)]