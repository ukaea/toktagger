...
def create_mock_data(file_path: str, shot_ids: int):
    data = {}
...
def main():
    ...
    project_id = create_project("New Mock Disruption Project", "disruption", "json", "uncertainty")
    ...
    non_annotated = create_mock_data("/data/test/non_annotated_data.json", list(range(num_samples + 1, (2*num_samples)+1)))
    create_local_samples(project_id, list(range(num_samples + 1, (2*num_samples)+1)), base_path="/data/test/non_annotated_data.json", file_type="json", signals=["ip"])
