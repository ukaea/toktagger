...
if __name__ == "__main__":
    ...
    async def _demo():
        client = AsyncMongitaClient("./demo_db")
        users = client["test"]["users"]
        await users.insert_one({"name": "Alice", "age": 30})
        await users.insert_one({"name": "Bob", "age": 22})
        print("find_one:", await users.find_one({"name": "Alice"}))
        print("count:", await users.count_documents({}))
        async for u in users.find(
            {"age": {"$gte": 18}}, sort=[("age", 1)], skip=0, limit=10
        ):
            print("iter:", u)
        await client.close()
