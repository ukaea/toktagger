...
async def get_samples(
    ...
    start: int = Query(0, description="Index of the first sample you want returned when sorted newest - oldest"), 
    end: int = Query(None, description="Index of the last sample you want returned when sorted newest - oldest, leave blank to return all entries")
    ) -> list[Sample]:
    """
...
@router.put(
    "",
    responses = {
        200: {"description": "Samples have been added successfully, and a list of their IDs has been returned."},
        404: {"description": "Project not found with that ID."},
    })
async def add_samples(
    request: Request, 
    samples: list[SampleIn],
    project_id: str = Path(description="The project ID to associate these samples with.")
    ):
    """
...
@router.get(
    ...
    responses = {
        200: {"description": "The next sample to annotate has been returned, according to the project's query strategy."},
        204: {"description": "No more samples are available to annotate for this project."},
...
async def get_sample(
    request: Request, 
    project_id: str = Path(description="The ID of the project to retrieve a sample from."), 
...
@router.delete(
    ...
    responses = {
        200: {"description": "Samples has been returned successfully."},
        404: {"description": "Sample not found with specified ID within selected project."},
    })
async def remove_sample(
    request: Request,
    project_id: str = Path(description="The ID of the project to delete a sample from."), 
