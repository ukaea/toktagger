...
async def get_all_annotations(
    request: Request, 
    project_id: str = Path(description="The ID of the project to retrieve annotations for"), 
    start: int = Query(0, description="Index of the first annotation you want returned when sorted newest - oldest"), 
    end: int = Query(None, description="Index of the last annotation you want returned when sorted newest - oldest, leave blank to return all entries"),
    validated: bool = Query(None, description="Whether to return only validated or unvalidated annotations, leave blank for all annotations")
    ) -> list[Annotation]: 
    """
...
async def delete_all_annotations(
    request: Request,
    project_id: str = Path(description="The ID of the project to delete all annotations for")):
    """
    Delete ALL annotations for the given project.
    ---------------------------------------------
    """
    # Delete annotations available for this project across all samples
    if not await request.app.state.db_client.get_document_by_id("projects", convert_to_objectid(project_id, "projects")):
        raise HTTPException(status_code=404, detail="Project not found with that ID.")
...
async def remove_annotations(
    ...
    sample_id: str = Path(description="The ID of the sample to delete annotations from."), 
    ):
    """
