...
export const DisruptionTable = () => {
    ...
    useEffect(() => {
        ...
        for (const [index, disruption] of disruptions.entries()) {
            const x0 = disruption.x.toFixed(6)
            const x1 = "--"
            const marker = "w-5 h-1.5 sm:rounded-lg"

            entriesBuffer.push(generateEntry(`disruption-${index}`, x0, x1, marker, disruption.category))
        }

        setEntries(entriesBuffer)
    }, [setEntries, zones, triggerZoneUpdate, disruptions, triggerDisruptionUpdate])
